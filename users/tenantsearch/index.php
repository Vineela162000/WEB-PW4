<?php
	...
	if(isset($_POST['delpref'])){
		if($TENANTPREF_EXISTS){
			mysqli_query($conn, "DELETE FROM tenantsearch WHERE '$ID'");
			$TENANTPREF_EXISTS = false;
		}
	}
	...
	if (isset($_POST['matchbtn'])) {
		...
		$result = mysqli_query($conn, "SELECT * FROM rentalspace");
		$num_rows = mysqli_num_rows($result);
		$row = mysqli_fetch_assoc($result);
		$RentArray = array();
	...
	function savePreference(){
		global $PREFPRICE, $PREFBOROUGH, $TENANTPREF_EXISTS, $ID, $conn;
	...
	class RentalSpace {
		...
		function calMagnitude(){
			...
			if($PREFBOROUGH != -1){ //if it's not any borough it will calculate the distance between two boroughs
				$result = mysqli_query($conn, "SELECT * FROM mtlborough WHERE BoroughID = '$this->borough'");
				...
				$result = mysqli_query($conn, "SELECT * FROM mtlborough WHERE BoroughID = '$PREFBOROUGH'");
				$row = mysqli_fetch_assoc($result);
			...
			<div class="collapse navbar-collapse" id="nav-bar-links">
				<?php
					...
					if ($_SESSION['login_USERTYPE'] == 'owner')
						print'
									<li><a href="#">Owners Listings</a></li>
									<li><a href-"#">Owners Preferences</a></li>
									<li class="divider"></li>
						';
					if ($_SESSION['login_USERTYPE'] == 'tenant')
						print'
									<li><a href="../tenantprofile">Tenants Profile</a></li>
	...
	<div class="container">
		<div class="well"> 
			<div class="row">
				...
				<!-- SEARCH SETTINGS -->
					<h3 style="display: block">Search for Properties
					...
					<?php 
						...
						if(!$TENANTPREF_EXISTS) print '<p style="color: red">Before you can find a home, you must first establish your preferences.</p>';
						else print '<p style="color: green">Your preferences are now set.</p>';
					?>
					<div id="SEARCH_SETTINGS" class="collapse in" style="margin-top:10px">
						<form  onSubmit="return validateSearch()" method="post" action="">
							...
							<label for="BOROUGH">Preferred Locality</label>
								<div class="row">
						...
						if ($MATCHBTNPUSHED) {
							...
							if($RentArray != null){
								for ($i=0; $i < count($RentArray) ; $i++) { 
									...
									$row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM rentalspace WHERE PostID = '$PostID_RentalSpace'"));
									print'
										<div class="row">
											...
											<div class="col-sm-9 col-lg-10">
												...
												<p>'.stripslashes($row['description']).'<br>';
													...
													<strong>Locality: </strong> '.$BOROUGHNAME[$row['borough']].' | <strong>Price: </strong>'.$row['price'].'$ Per Month <br>
												</p>
											</div>
										</div>
										<hr>
